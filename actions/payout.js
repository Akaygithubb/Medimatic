...
export async function requestPayout(formData) {
  ...
  try {
    ...
    //check if doctor has any pending payout request

    const existingPayoutRequest = await db.payout.findFirst({
      where: {
        doctorId: doctor.id,
        status: "PROCESSING",
      },
    ...
    if (existingPayoutRequest) {
      throw new Error(
        "You already have a pending payout request. Please wait it for to be processed"
      );
    }

    //get doctor current credit's balance
