...
const AppointmentCard = ({ appointment, userRole }) => {
  ...
  const handleMarkCompleted = async () => {
    ...
    if (
      window.confirm(
        "Are your sure you want to mark this appointment as completed? This action can't be undone."
      )
    ) {
  ...
  const handleCancelAppointment = async () => {
    ...
    if (
      window.confirm(
        "Are your sure you want to cancel this appointment? This action can't be undone."
      )
    ) {
      const formData = new FormData();
      formData.append("appointmentId", appointment.id);
  ...
  return (
    <>
      <Card className="border-emerald-900/ hover:border-emerald-700/30 transition-all ">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="font-medium text-white">
                  {userRole === "DOCTOR"
                    ? otherParty.name || otherParty.email || "Unnamed"
                    : `Dr. ${otherParty.name || "Unnamed"}`}
                </h3>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-2xl">
                ...
                {userRole === "DOCTOR" &&
                  ...
                  appointment.status !== "CANCELLED" && (
                    <Button
                      ...
                      className="text-emerald-400 h-7 hover:text-emerald-300 hover:bg-emerald-900/20"
                    >
          ...
          <DialogFooter className="flex flex-col-reverse sm:flex-row sm:justify-between sm:space-x-2">
            {/* cancel button for sheduled appointments */}
